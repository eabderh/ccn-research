...
def buildcfg(targets, routeinfo):
    for target in targets:
        node_cfg = buildcfg_node(target, routeinfo)
        #print target
        #print node_cfg
        #raw_input()
        f = open("metis.cfg", 'w')
        f.write(node_cfg)
        f.close()
        command = "scp metis.cfg " + target + ":."
        print command
        call(command, shell=True)
    commands = ["sudo cp metis.cfg CCNx_Distillery/usr/bin/."]
    exec_ssh_commands(targets, commands)

def buildcfg_node(target, routeinfo):
    ...
    string = "add listener tcp local0 127.0.0.1 9695\n\n"
    for n, connection in enumerate(routeinfo[target]):
        localip = routeinfo[target][connection]
        remoteip = routeinfo[connection][target]
        string += conntemplate1.format(int=n, localip=localip)
        string +=  conntemplate2.format( localip=localip, \
                remoteip=remoteip, \
                remotehost=connection)
    string += "\n"
    for n, connection in enumerate(routeinfo[target]):
        string += routetemplate.format(remotehost=connection)
    string += "\n"
    string += "list connections\nlist routes\n"
    return string

#def buildcfg(target, routeinfo):
#    conntemplate1 = "add listener udp udp{int} {localip} 9695\n"
#    conntemplate2 = "add connection udp conn{remotehost} {remoteip} 9695 {localip} 9695\n"
#    routetemplate = "add route conn{remotehost} lci:/ 1\n"
#    print routeinfo
